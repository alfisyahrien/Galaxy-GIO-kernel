...
/*
* * * Index in the cpufreq_frequency_table for the sweet spot.
* * This is ALWAYS the speed step 1 below policy max speed.
* */
static int sweet_spot_index;
...
unsigned int sweet_spot_skip; /* Threshold to stay in sweet spot. */
};
...
return 0;
#if 0 /* It may stay in certain step. So,
it is better to keep original request value. */
...
/*
* Sweet spot as a weight of SWEET_SPOT_THRESHOLD which is
* how many times longer to say in this frequency before
* going to the next.
*/
...
freq_target = (dbs_tuners_ins.freq_step *
...
/* Reset sweet spot counter on clock down. */
this_dbs_info->sweet_spot_skip = 0;
...
dbs_check_cpu(i);
    ...
    /* Replace timer from fix period to getting next wake
* timer wake up time, to prevent unnecessary interrupt
* */
dbs_tuners_ins.sampling_rate =
jiffies_to_usecs(get_next_timer_interrupt(jiffies) - jiffies);
...
struct cpufreq_governor cpufreq_gov_msm7k = {
.name = "msm7k",
.governor = cpufreq_governor_dbs,
.max_transition_latency = TRANSITION_LATENCY_LIMIT,
.owner = THIS_MODULE,
};
EXPORT_SYMBOL(cpufreq_gov_msm7k);

static int __init cpufreq_gov_dbs_init(void)
{
return cpufreq_register_governor(&cpufreq_gov_msm7k);
}
...
cpufreq_unregister_governor(&cpufreq_gov_msm7k);
}
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_MSM7K
